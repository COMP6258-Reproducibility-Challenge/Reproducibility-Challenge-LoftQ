...
"""
This module is exactly the same as the run_loftq module, except that it should be run with the 
gsm8k dataset or other QA datasets. This module depends on the qa_utils module
"""
...
if __name__ == "__main__":
    ...
    num_labels, labels = qa_utils.count_labels(raw_data)
    ...
    if not training_args.no_train:
        if not training_args.train_small is None and training_args.train_small == True:
            raw_data['train'] = qa_utils.create_reduced_dataset(raw_data["train"], labels, num_examples=100)
            if data_args.task_name == "mnli":
                raw_data['validation_matched'] = qa_utils.create_reduced_dataset(raw_data["validation_matched"], labels, num_examples=3)
                raw_data['validation_mismatched'] = qa_utils.create_reduced_dataset(raw_data["validation_mismatched"], labels, num_examples=3)
            else:
                raw_data['validation'] = qa_utils.create_reduced_dataset(raw_data["validation"], labels,num_examples=3)
