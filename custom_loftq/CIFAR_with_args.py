...
# LoftQ imports
from model_utils import estimate_model_size, convert_linear_layer, convert_true_quant_conv_layer
...
def main():
    ...
    loftq_args = MockModelArgs(
        ...
        reduced_rank=model_args.rank, 
        quant_method=model_args.method
    )
    ...
    try:
        ...
        quantized_model = quantize_resnet_model(
            model_to_quantize,
            loftq_args,
            quantize_final_fc_layer=model_args.final_layer
        )
        ...
        quantized_model = train_model(
            ...
            optimizer, 
            epochs=300, 
            model_path=f'resnet50_cifar10_quantized_b{model_args.bits}_r{model_args.rank}_{model_args.method}_{model_args.final_layer}.pth',
            scheduler=scheduler
        )
        ...
        accelerator.print(f"ERROR during quantization/evaluation for {model_args.bits}-bit, rank {model_args.rank}, {model_args.method}, final_quant={model_args.final_layer}: {e}")
        import traceback
        traceback.print_exc()
        accuracy = "Error"
    ...
    if accelerator.is_main_process:
        with open(csv_file_path, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([model_args.bits, model_args.rank, model_args.method, model_args.final_layer, accuracy, f"{time_taken:.2f}"])
